...
// 裁切图片,按坐标或自动
    ...
    public function turn(){
        $this -> TRUE_COLOR = imagecreatetruecolor($this -> PICTURE_WIDTH, $this -> PICTURE_HEIGHT);
        $WHITE = imagecolorallocate($this -> TRUE_COLOR, 255, 255, 255);
        imagecolortransparent($this -> TRUE_COLOR,$WHITE);
        imagefill($this -> TRUE_COLOR,0,0,$WHITE);
        $rote = imagerotate ($this -> TRUE_COLOR, $this -> CIRCUMROTATE, 0);

        imageCopyResized($rote, $this -> PICTURE_CREATE, 0, 0, 0, 0, $this -> PICTURE_WIDTH, $this -> PICTURE_HEIGHT, $this -> PICTURE_WIDTH, $this -> PICTURE_HEIGHT);

        return imagejpeg($rote,$this -> PIC_URL);
        ...
        return imagerotate ($this -> TRUE_COLOR, $this -> CIRCUMROTATE, $WHITE);
    }
    /**
     * 图片锐化
     */
